...
def main(spark, df_train, df_val, model_file):
    ...
    train = train.withColumn("count", log(train["count"] + 1))
    print("log-transformed train data")    
	
    # import validation data
    val = spark.read.parquet(df_val)
    print("imported validation data")
    # log transform validation data
    val = val.withColumn("count", log(val["count"] + 1))
    print("log-transformed validation data")
    ...
    # Build the recommendation model using ALS on the train data. Note we set cold start strategy to 'drop' to ensure we don't get NaN evaluation metrics
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("extension1_log_train").getOrCreate()
